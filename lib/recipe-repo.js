...
export function init(recipes, triggers = {}) {
  ...
  for (const recipe of Object.keys(triggers).concat(recipes)) {
    if (recipe.namespace === "sub-sequence") {
      executionDelays[`trigger.${recipe.namespace}.${recipe.name}`] = recipe.executionDelay;
    }
  }
  ...
  recipes.forEach((recipe) => {
    ...
    recipe.sequence.forEach((step) => {
      ...
      if (key.startsWith(".")) {
        ...
        if (![ "string", "function" ].includes(typeof fn)) {
  ...
  return {
    ...
    triggerHandler: async (routingKey) => {
      return await handlerFunction(triggers[routingKey]);
    },
    executionDelay: (routingKey) => executionDelays[routingKey],
    genericTriggerUsesParentCorrelationId: (routingKey) => useParentCorrelationIdMap[routingKey],
    unrecoverableHandler: async (routingKey) => {
      return await handlerFunction(unrecoverableMap[routingKey]);
    },
    handler: async (routingKey) => {
      const fn = lambdaMap[routingKey] || (borrowedKeys[routingKey] && lambdaMap[borrowedKeys[routingKey]]);
      return await handlerFunction(fn);
    },
  };
}

async function handlerFunction(fn) {
  if (!fn) return;
  if (typeof fn === "function") {
    return fn;
  }

  const fnPath = path.resolve(process.cwd(), fn);
  const dynamicImport = await import(fnPath);
  if (!(dynamicImport && dynamicImport.default)) {
    const err = new Error(`Could not import default function from path: ${fnPath}`);
    err.rejected = true;
    throw err;
  }
