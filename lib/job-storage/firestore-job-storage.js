...
async function completedChild(child, { correlationId, parentCorrelationId, key }) {
  ...
  try {
    const res = await db.runTransaction(async (t) => {
      ...
      // so we'll get the current processed doc first (need it to know how many children to expect)
      const processedRef = await db.collection("processed").doc(parentCorrelationId);
      const doc = await t.get(processedRef);
      const parentData = doc.data();
      logger.info(`Expecting ${parentData?.startedJobs?.length} children`);

      // then we'll check if we're done
      const { allChildrenComplete, completedJobs } = completedCheck(parentCorrelationId, correlationId, parentData, logger);
      logger.info(`Completed ${completedJobs?.length} jobs`);
      logger.info(`All children complete: ${allChildrenComplete}`);

      // then we'll update the bucket with the new completed job
      ...
      // now we're done with our reads, so we start on the writes
      if (!bucketData) {
      ...
      // and if we are then we'll update the parent doc
      if (allChildrenComplete) await t.update(processedRef, { completedJobs });

      return [ allChildrenComplete, parentData, completedJobs?.length ];
    });
    return res;
  } catch (e) {
...
async function completedCheck(parentCorrelationId, correlationId, parentData, logger) {
  ...
  return {
    allChildrenComplete: completedJobs?.length === parentData?.startedJobs?.length,
    completedJobs,
  };
}
