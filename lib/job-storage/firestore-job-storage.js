...
export async function storeParent(parentCorrelationId, children, message, nextKey) {
...
export async function completedChild({ correlationId, parentCorrelationId, key }) {
...
export async function parentIsComplete({ parentCorrelationId, key, siblingCount }) {
...
export async function completedCheck(parentCorrelationId) {
...
export async function removeParent(parentCorrelationId) {
...
export async function messageAlreadySeen(idempotencyKey, deliveryAttempt) {
  try {
    await db
...
export async function getFromDb(key, collectionName = "processed") {
  const doc = await db.collection(collectionName).doc(key).get();
  return doc.data();
}

export async function getAllFromDb(collectionName = "processed") {
  const collection = await db.collection(collectionName).get();
  return Object.fromEntries(collection.docs.map((doc) => [ doc.id, doc.data() ]));
}
