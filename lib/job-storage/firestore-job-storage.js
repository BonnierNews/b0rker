...
async function storeParent(parentCorrelationId, children, message, nextKey) {
  logger.info(
    `Storing parent ${parentCorrelationId} with ${children?.length} children`,
    { correlationId: parentCorrelationId, key: "storeParent" }
  );
...
async function parentIsComplete({ parentCorrelationId, key, siblingCount }) {
  try {
    ...
    logger.info(
      ...
      { correlationId: parentCorrelationId, key }
    );
    ...
    logger.error(`Parent is complete failed ${e}`, { correlationId: parentCorrelationId, key });
    throw e;
  }
}
...
async function removeParent(parentCorrelationId) {
  try {
    ...
    logger.error(`Remove parent failed ${e}`, { correlationId: parentCorrelationId, key: "removeParent" });
    throw e;
  }
}
