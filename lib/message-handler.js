  ...
  if (suffix === "unrecoverable") {
    ...
    if (sequence === "sub-sequence" && parentCorrelationId) {
      ...
      try {
        ...
        if (isLast) {
          // TODO: SIVA we should delete the job that was stored here
          await publishMessage({
            ...parentData.message,
        ...
        logger.error(`sub-sequence triggered from parent is finished but calling job storage to complete child failed: ${error}`);
