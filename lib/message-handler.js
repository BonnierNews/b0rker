...
export default async function messageHandler(recipeMap, req, res) {
  ...
  if (suffix === "unrecoverable") {
    ...
    if (sequence === "sub-sequence" && parentCorrelationId) {
      ...
      try {
        const [ isLast, parentData, completedJobCount ] = await jobStorage.completedChild(message, { parentCorrelationId, correlationId, key });
        if (isLast) {
          await publishMessage({
            ...parentData.message,
            data: [ ...parentData.message.data, { type: key, id: completedJobCount } ],
          },
          { key: parentData.nextKey, correlationId, parentCorrelationId });
          // TODO: we should delete the job that was stored here
        }
      } catch (error) {
        // if we get here then we're in an unknown state - the child has completed, but we haven't recorded that
        logger.error(`sub-sequence triggered from parent is finished but calling job storage to complete child failed: ${JSON.stringify(error)}`);
    ...
    switch (result?.type) {
      ...
      case "trigger":
        if (result.key.startsWith("sub-sequence")) {
          ...
          await publishMessagesBulk(result.messages.map(formatMessage), { key: triggerKey, correlationId, parentCorrelationId });
...
async function startChildProcesses(result, parentCorrelationId, nextKey, message) {
  const children = result.messages.map((o) => ({ message: o, attributes: { parentCorrelationId, correlationId: v4() } }));
