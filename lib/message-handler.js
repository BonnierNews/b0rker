...
export default async function messageHandler(recipeMap, req, res) {
  ...
  if (suffix === "unrecoverable") {
    ...
    if (sequence === "sub-sequence" && parentCorrelationId) {
      ...
      const [ isLast, parentData ] = await jobStorage.completedChild(message, { parentCorrelationId, correlationId, key });
      if (isLast) {
    ...
    switch (result?.type) {
      ...
      case "trigger":
        if (result.key.startsWith("sub-sequence")) {
          ...
          await publishMessagesBulk(result.triggerMessages.map(formatMessage), { key: triggerKey, correlationId, parentCorrelationId });
...
function appendData(data, result) {
  const newData = [ ...data ];
  if (result) {
    if (result?.triggerMessages) {
      newData.push({ type: result.key, id: result.triggerMessages.length });
...
function isValidResult(result, logger) {
  if (result?.type === "trigger") {
    if (!result.key) {
      ...
      return false;
    } else if (result.key.startsWith("sequence") && !(result.triggerMessages && Array.isArray(result.triggerMessages))) {
