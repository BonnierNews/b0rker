...
export default async function messageHandler(recipeMap, req, res) {
  ...
  const { key, parentCorrelationId, grandParentCorrelationId, siblingCount, parentSiblingCount, idempotencyKey } =
  ...
  try {
      ...
      case "trigger":
        if (result.key.startsWith("sub-sequence")) {
          ...
          if (grandParentCorrelationId) {
            ...
            error.rejected = true;
            error.extraMessage = tooDeepMessage;
    ...
    await publishMessage(
      { ...message, data },
      { key: nextStep, correlationId, parentCorrelationId, grandParentCorrelationId, siblingCount, parentSiblingCount }
    );
