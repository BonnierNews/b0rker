...
export default async function messageHandler(recipeMap, req, res) {
  ...
  if (prefix === "trigger") {
    const triggerHandler = await recipeMap.triggerHandler(key);
  ...
  if (suffix === "unrecoverable") {
    ...
    const unrecoverableHandler = await recipeMap.unrecoverableHandler(prevKey);
  ...
  try {
    const handler = await recipeMap.handler(key);
