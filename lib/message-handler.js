...
export default async function messageHandler(recipeMap, req, res) {
  ...
  if (prefix === "trigger") {
    ...
    if (Array.isArray(messages)) {
      ...
      await publishMessage(formatMessage(message), { key: firstStep, correlationId, parentCorrelationId });
    }
  ...
  if (suffix === "unrecoverable") {
    ...
    if (sequence === "sub-sequence" && parentCorrelationId) {
      ...
      try {
        const [ isLast, parentData, completedJobCount ] = await jobStorage.completedChild(message, {
          parentCorrelationId,
          correlationId,
          key,
        });
  ...
  try {
      ...
      case "resume":
        if (Number(resumedCount) >= maxResumeCount) {
          ...
          await rejectMessage(
            ...
            { key, correlationId, parentCorrelationId }
          );
          return res.status(200).send();
        }
    ...
    if (error.unrecoverable) {
      try {
        ...
        await publishMessage(
          { ...message, error: { message: error.extraMessage } },
          { key: `${key}.unrecoverable`, correlationId, parentCorrelationId }
        );
        return res.status(200).send();
      } catch (publishErr) {
        logger.error(
