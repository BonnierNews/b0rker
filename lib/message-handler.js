...
export default async function messageHandler(recipeMap, req, res) {
  ...
  if (prefix === "trigger") {
    ...
    if (error) {
      logger.error(
        `Got error running trigger for message with correlationId: ${correlationId}. Error: ${error}. Message: ${JSON.stringify(message)}`
      );
      return res.status(400).send();
    }
  ...
  const delayKey = key.split(".").slice(0, 2).join(".");
  if (recipeMap.executionDelay(delayKey)) {
    await sleep(recipeMap.executionDelay(delayKey));
  }
  try {
    ...
    if (error.rejected) {
      logger.error(`Rejected message with correlationId: ${correlationId}. Error: ${error}. Message: ${JSON.stringify(message)}`);
      await rejectMessage(
    ...
    if (error.retry) {
      logger.warn(
        `Retrying message ${messageId} with correlationId: :${correlationId}. Error: ${error}. Message: ${JSON.stringify(message)}`
      );
    ...
    if (error.unrecoverable) {
      try {
        ...
        logger.error(
          `Unable to handle unrecoverable message with correlationId: ${correlationId}. Error: ${error}. Message: ${JSON.stringify(message)}`
        );
        return res.status(500).send();
      }
    }
...
function isValidResult(result, logger) {
  if (result?.type === "trigger") {
    if (!result.key) {
      ...
      logger.error(`Invalid result: invalid messages ${JSON.stringify(result.messages)} for handler that returns a trigger`);
      return false;
    }
  }
  return true;
