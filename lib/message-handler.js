...
import publishMessage, { rejectMessage, publishMessagesBulk, publishChildren } from "./publish-message.js";
...
async function startChildProcesses(result, parentCorrelationId, nextKey, message) {
  const siblingCount = result.messages.length;
  const children = result.messages.map((o) => ({
    ...
    attributes: {
      ...
      siblingCount: siblingCount.toString(), // because firestore needs it to be a string},
    },
  }));
  await jobStorage.storeParent(parentCorrelationId, children, message, nextKey);
  await publishChildren(children);
}
