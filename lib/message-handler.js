...
export default async function messageHandler(recipeMap, req, res) {
  ...
  if (parts.shift() === "trigger") {
    ...
    await publishMessage({ ...message, data: [ ...(message.data ?? []) ] }, { key: firstStep, correlationId, parentCorrelationId: parentCorrelationId || "" });
  ...
  if (parts.pop() === "processed") {
    ...
    if (sequence === "sub-sequence" && parentCorrelationId) {
      ...
      if (isLast) {
        await publishMessage(parentData.message, { key: parentData.nextKey, correlationId, parentCorrelationId: parentCorrelationId || "" });
  ...
  try {
    ...
    await publishMessage({ ...message, data: newData }, { key: nextStep, correlationId, parentCorrelationId: parentCorrelationId || "" });
...
async function startChildProccesses(result, parentCorrelationId, nextKey, message) {
  ...
  for (const child of children) {
    await publishMessage(child.message, { key: `trigger.${result.key}`, ...child.attributes });
  }
}
