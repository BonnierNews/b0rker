...
export default async function messageHandler(recipeMap, req, res) {
  ...
  const {key, resumedCount} = messageData.attributes;
  ...
  if (parts.pop() === "processed") {
    ...
    const {parentCorrelationId} = message;
    ...
    if (sequence === "sub-sequence" && parentCorrelationId) {
      ...
      const [isLast, parentData] = await jobStorage.completedChild(message);
      if (isLast) {
        await publishMessage(parentData.message, {key: parentData.nextKey});
  ...
  try {
    ...
    switch (result?.type) {
      ...
      case "trigger":
        await startChildProccesses(result, `${key}:${correlationId}`, recipeMap.next(key), {...message, data: newData});
        return res.status(200).send();
      default:
        break;
    }
    ...
    await publishMessage({...message, data: newData}, {key: nextStep});

    return res.status(200).send();
  } catch (error) {
    if (error.rejected) {
      logger.error(`Rejected message with correlatioId: ${correlationId}. Error: ${error}. Message: ${message}`);
...
async function startChildProccesses(result, parentCorrelationId, nextKey, message) {
  const children = result.source.map((s) => ({...s, parentCorrelationId, correlationId: uuid()}));
  await jobStorage.storeParent(parentCorrelationId, children, message, nextKey);
  for (const child of children) {
    await publishMessage(child, {key: `trigger.${result.key}`});
  }
}
