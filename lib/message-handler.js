...
export default async function messageHandler(recipeMap, req, res) {
  ...
  if (prefix === "trigger") {
    ...
    if (rest.length === 0) {
      ...
      if (!triggerHandler) {
        logger.error(`Got trigger message for an unknown trigger handler, ${messageId}`);
        return res.status(400).send();
      }

      const result = await triggerHandler(message, context);

      if (!isValidResult(result, logger)) {
        logger.error(`Got invalid result from trigger handler on key: ${key}, result: ${JSON.stringify(result)}`);
        return res.status(400).send();
      }
      triggerKey = result.key;
    }
