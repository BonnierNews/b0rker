...
function startSequence({ key: firstKeyInSequence, queue }) {
  return async (req, res) => {
    const { correlationId, parentCorrelationId, grandParentCorrelationId, siblingCount, parentSiblingCount } =
      req.attributes;
    await publishTask(
      `${req.attributes.relativeUrl}/${firstKeyInSequence.replace(/^\./, "")}`,
      appendData(req.body, []), // Ensure we have a data element
      { correlationId, parentCorrelationId, grandParentCorrelationId, siblingCount, parentSiblingCount },
      queue
    );
...
function messageHandler(func, { nextKey, queue } = {}) {
  return async (req, res) => {
    const { key, correlationId, parentCorrelationId, grandParentCorrelationId, siblingCount, parentSiblingCount } =
      req.attributes;
...
async function handleTriggerResult(
  ...
  body,
  { nextKey, queue },
  { key, correlationId, parentCorrelationId, grandParentCorrelationId, siblingCount, parentSiblingCount }
) {
  ...
  if (result.key.startsWith("sub-sequence")) {
    if (result.messages.length > 0) {
      if (grandParentCorrelationId) {
        const tooDeepMessage = `It is only possible to nest one level of sub-sequences, you're trying to trigger ${result.key} from ${key} which in turn was triggered from ${parentCorrelationId.split(":").shift()} - either rethink what you're trying to do, or implement great-grandchilden in b0rker...`;
