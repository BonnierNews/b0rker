...
function buildCloudTaskSequenceRoutes(router, { namespace, name, sequence, unrecoverable }, nextKeyMapper) {
  ...
  router.post(`/${namespace}/${name}`, sequenceTrigger(sequenceIterator(sequence).next().value[0]));
  ...
  return async (req, res) => {
    ...
    if (result?.type === "trigger") {
      ...
      if (errorMessage) {
        ...
        return res.status(400).send({ type: "error", message: errorMessage });
      }
      ...
      if (result.key.startsWith("sub-sequence")) {
        ...
        await triggerOtherSequences(req.body, result, { correlationId, parentCorrelationId, siblingCount });
      } else {
        logger.error(`Invalid trigger key ${result.key} returned from handler`);
        return res.status(400).send({ type: "error", message: `Invalid trigger key ${result.key}` });
      }
    }
...
function checkForTriggerError(result) {
  ...
  if (result.key.includes("sub-sequence") && !(result.messages && Array.isArray(result.messages))) {
    return `Invalid result: messages need to be an array got: ${JSON.stringify(result.messages)}`;
  }
  if (result.key.startsWith("sequence") && !(result.messages && Array.isArray(result.messages))) {
    return "Invalid result: publishMessages must be an array";
  }
...
function sequenceTrigger(firstKeyInSequence) {
  return async (req, res) => {
...
async function triggerOtherSequences(originalMessage, result, { parentCorrelationId, correlationId, siblingCount }) {
  ...
  await publishTasksBulk(keyToUrl(result.key), messages, { correlationIdStrategy: "new" });
}
