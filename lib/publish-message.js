...
export async function publishChildren(messages) {
  const client = await pubSubClient.topic(config.topic, {
    batching: {
      maxMessages: 1000,
      maxMilliseconds: 10,
  ...
  const promises = messages.map(({ message, attributes }) =>
    (async () => {
      const logger = buildLogger(attributes.correlationId);
      const messageId = await publishPubsubMessage(
        client, {
          json: message,
          attributes: cleanupAttributes({ ...attributes, topic: config.topic, idempotencyKey: uuid.v4() }),
        },
        logger
      );
      logger.info(`Published message ${messageId}`);
