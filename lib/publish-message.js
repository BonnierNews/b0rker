...
export default async function publishMessage(message, attributes) {
  const messageId = await pubSubClient.topic(config.topic).publishMessage({ json: message, attributes: { ...attributes, topic: config.topic } });
  buildLogger(message.correlationId).info(`Published message ${messageId}`);
}

export async function rejectMessage(message, attributes) {
  const messageId = await pubSubClient.topic(config.deadLetterTopic).publishMessage({ json: message, attributes: { ...attributes, topic: config.topic } });
